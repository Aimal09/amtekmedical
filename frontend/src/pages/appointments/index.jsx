...
export default function Appointments() {
  ...
  const [endTime, setEndTime] = useState(
    moment().add(1, "hours").format("HH:00")
  );
  ...
  const fetchAppointmentData = async () => {
    ...
    const _schedules = await CallApi("GetAppointmentsByDate", "GET", {
      startDate,
      endDate,
    });
    ...
    if (localStorage.role === "3") {
      ...
      const filteredSchedules = _schedules.filter(
        (schedule) => String(schedule.DoctorId) === String(doctorId)
      );
      setAppointmentData(filteredSchedules);
    } else {
  ...
  const setCalendar = () => {
    dates &&
      dates.forEach((d) => {
        const appointments =
          appointmentData &&
          appointmentData.filter((f) => {
            return moment.utc(f.AppointmentDate).date() == d;
          });
        ...
        appointments &&
          appointments.forEach((ap) => {
            ...
            const id = ap.Id;
            const isViewd = ap.IsViewd != null;
            const endTime =
              startTime + (_end.diff(_start, "minutes") / 60) * 100;
            if (!appointmentObj.hasOwnProperty(index))
              appointmentObj[index] = [];
            appointmentObj[index].push({
              title,
              startTime,
              endTime,
              id,
            ...
            <div>
              ...
              <div className="form-group d-flex flex-column">
                ...
                <DatePicker
                  ...
                  onChange={(e) =>
                    setAppointmentDate(moment(e.toDate()).format("YYYY-MM-DD"))
                  }
                />
              </div>
              ...
              {showFilters && (
                <div className="doctors">
                  {doctors &&
                    doctors.map((doctor) => (
                      <p key={doctor.Id}>
                        <input type="checkbox" id={doctor.Id} />
                        <label htmlFor={doctor.Id}>
                          {doctor.Id} {doctor.Name}
                        </label>
                      </p>
                    ))}
                  <span className="butn d-block mx-auto mt-3 text-center">
                    Apply
                  </span>
                </div>
              )}
