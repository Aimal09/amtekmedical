...
const ScheduleDetails = () => {
    ...
    // useEffect(() => {
    //     if (appoinment && Object.keys(appoinment).length > 0) {
    //         console.log("Updating State with Data:", appoinment);
    //         setName(appoinment.PatientName || ""); 
    //         setAge(appoinment.Age || ""); 
    //         setGender(appoinment.gender || ""); 
    //         setNationality(appoinment.Nationality || ""); 
    //         setAddress(appoinment.address || ""); 
    //         setPhoneNumber(appoinment.Contact || ""); 
    //     }
    // }, [appoinment]); // Jab bhi data aye, state update ho
    ...
    useEffect(() => {
        ...
        if (appoinmentId) { // Agar ID available hai tabhi call karo
            ...
            <div className="card p-4 shadow-lg">
                <div className="row">
                    ...
                    <div className="col-md-4">
                        ...
                        <p className="fs-5"><strong>Name:</strong> {appoinment.Name}</p>
                        <p className="fs-5"><strong>Age:</strong> {appoinment.Age}</p>
                        {/* <p className="fs-5"><strong>Gender:</strong> {appoinment.gender}</p> */}
                        <p className="fs-5"><strong>Nationality:</strong> {appoinment.Nationality}</p>
                        {/* <p className="fs-5"><strong>Address:</strong> {appoinment.address}</p> */}
                        <p className="fs-5"><strong>Phone Number:</strong> {appoinment.Contact}</p>
                        <p className="fs-5"><strong>Heart Problem:</strong> {appoinment.healthProblem?"Yes":"No"}</p>
                        <p className="fs-5"><strong>Allergy Problems :</strong> {appoinment.allergyProblems?"Yes":"No"}</p>
                        <p className="fs-5"><strong>Blood Pressure:</strong> {appoinment.bloodPresure?"Yes":"No"}</p>
                        <p className="fs-5"><strong>Thyroid Problems :</strong> {appoinment.thyroidProblems?"Yes":"No"}</p>
                        <p className="fs-5"><strong>Asthama:</strong> {appoinment.asthama?"Yes":"No"}</p>
                        <p className="fs-5"><strong>Medication:</strong> {appoinment.medication}</p>
                        <p className="fs-5"><strong>AnyOther Problem:</strong> {appoinment.anyOther}</p>
                        <p className="fs-5"><strong>Pregnant :</strong> {appoinment.pregnant?"Yes":"No"}</p>
                    </div>
...
<div>
    ...
    />
                        ...
                        <button type="button" className='butn-sec mt-4 mr-4' onClick={appoinmentForm}>Save</button>
                ...
                <div className="mt-3">
                 ...
                 <table className="mt-3 table table-striped">
                        ...
                        <tbody>
                            {appoinmentHistory && appoinmentHistory.map(app => 
            ...
            <div className="card p-4 shadow-lg mt-4">
                    ...
                    <p><strong>Treatment:</strong> {selectedVisit.treatment}</p>
                    <p><strong>Amount:</strong> {selectedVisit.amount}</p>
                    <button className="btn btn-secondary" onClick={() => setSelectedVisit(null)}>Close</button>
                </div>
                )}
        </div>
    );
